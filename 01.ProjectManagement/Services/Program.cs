...
namespace _01.ProjectManagement.Services
{
    internal class Program
    {
        ...
        static void AddProject()
        {
            ...
            {
                Console.WriteLine("Грешка: лидерът трябва да бъде с роля 'Leader'. Моля, въведете отново.");
                leader = ReadWorker();
            }
        ...
        static List<Worker> AddWorkers()
        {
            ...
            while(true) 
            {
                ...
                if(selectedOption == 1) 
                {
                    if(workers.Count < 5)
                    {
                        Worker dev = ReadWorker();
                        if (dev.Role == Role.Leader)
                        {
                            Console.WriteLine("Грешка: само един лидер е позволен на проект. Избери друга роля.");
                        }
                        else
                        {
                            workers.Add(dev);
        ...
        static Worker ReadWorker()
        {
            ...
            Console.Write("Избор (число): ");
            int roleChoice = int.Parse(Console.ReadLine());

            Role roleSelected = (Role)roleChoice;
        ...
        static void PrintProjectsSortedByStartDate()
        {
            ...
            foreach (var p in sorted)
            {
                ...
                {
                    Console.WriteLine($"  - {dev.Name}, {dev.Role}");
                }
            }
        }
